...
env.reset(options={"random":False})
print("Joints:",env.state,"ee-Position:",env.output[:3],'ee-Orientation:',env.output[3:])
n=100
for ep in range(n):
    state,_ = env.reset(options={"random":True})
    for i in range(3000):
        action = -0.03*env.robot.inverse_kinematics(5,env.output[:3],env.output[3:])
